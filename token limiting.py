...
def bias_mitigation_activity():
    print("\n===CREATIVE TRANSLATION ACTIVITY===\n")
    prompt=input("Enter a prompt to explore the formal tone (eg:'Describe a drawing to someone who can't see it'):").strip()
    if not prompt:
        print("Please enter a prompt to run the activity.")
        return
    
    initial_response=generate_response(prompt,temperature=0.1,max_tokens=1111)
    print(f"\nInitial AI Response:{initial_response}")
    modified_prompt=input("\n\nModify the prompt to make it more creative(eg:'Carry a hidden world from canvas to consciousness, without relying on sight'):").strip()

    if modified_prompt:
        modified_response=generate_response(modified_prompt,temperature=0.9,max_tokens=1111)
        print(f"\nModified AI Response(Creative):{modified_response}")
